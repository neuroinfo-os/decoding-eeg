"""
This script was copied from github.com/tru-hy/rpeakdetect
and adjusted to Python3. It's not perfect but suits our affordances.
"""
...
def detect_beats(
    ...
    #shannon_energy = np.array([calc_inf(p) for p in square_decg_power])
    shannon_energy = calc_information(square_decg_power)
...
def calc_information(p):
    if p==0:
        ...
        return -p*np.log(p)
calc_information = np.vectorize(calc_information)
